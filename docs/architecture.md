...
`sql` and `kv` are the facet's own Durable Object storage, bound by `bind(ctx)` in the generated entry, which is why `@std` throws outside a handler. `blob` is a scoped `Blobs` capability on the router, a `WorkerEntrypoint` with plain methods like `Logs`, that prefixes every key with the applet's id so no applet reaches another's. Values cross the RPC boundary as bytes, not a stream, so a blob is held in memory on both sides; streaming over RPC is a later improvement, and the docs tell applets to keep blobs to a few megabytes.
...
- an await on anything else, such as an outbound `fetch()`, lets another request in at that point
- writes are confirmed once per response: the response is held until the storage it depends on is durable

So the rule the docs give applets, database work in straight-line sync code and network calls before or after it, is this model stated from the inside.

### Schedules

The supervisor owns an applet's schedule: setting the expression stores it in the supervisor and arms a Durable Object alarm for the next occurrence, and the alarm calls the applet's `scheduled` export through the facet, records the run, then rearms. Expressions are five fields, minute resolution, UTC, parsed by Effect's `Cron` module; the settings refuse one it rejects. 'run now' and `applet_run` call the same path with the same event at once.
...
One pure function, `can(subject, action, applet)` in `packages/router/src/policy.ts`, turns a subject's role and an applet's owner and visibility into yes or no; nothing else in the router decides access, and the table it implements is under "Access" in [platform.md](platform.md#access). The admin is the address in `OWNER_EMAIL`, set at deploy and not stored; a user is an email in the `users` table; an unauthenticated request is `null`. `GET /applets` lists every applet for the admin with `failures`, the failed runs of the last 24 hours, counted on the caller's own applets only. The boundary that keeps the admin out of another user's code and logs is in the API and the editor, not in cryptography, since the admin holds the Cloudflare account.

Identity comes from [Better Auth](https://www.better-auth.com), running inside the router on the registry database, with a magic link as the only way in: the sign-in page takes an email, the router sends a one-time link through its email binding, valid for an hour, and pressing the button on the page it opens sets the session cookie. The link lands on a page rather than the verify route because mail scanners such as Outlook's Safe Links open every link they see, and the token is spent on first use. An address outside `users` gets no email and no Better Auth row, and the page says the same thing either way. The first sign-in of an allowed address creates its Better Auth row; one-time setup for a new person would go in `databaseHooks.user.create.after`, and nothing needs it today. Removing a user deletes their row, their sessions and their API keys.

A session lasts 30 days and slides: any request after a day of use pushes the expiry out another 30 days. A signed cookie caches the Better Auth session for 5 minutes, and the router still checks `users` on every authenticated request, so the cache does not delay removal. Cookies are set for the host suffix, so one sign-in covers every applet subdomain. That needs a real suffix: `*.localhost` cannot share a cookie, so sign-in is only exercised on the public host.

An API key is stored as its SHA-256 hash with the creator's email, a name, and when it was made and last used. `subject(headers)` in `auth.ts` is the one place a request becomes a subject: a bearer credential first, which is an OAuth access token when it has a JWT's three parts and an API key otherwise, else the session, else `null`. `ADMIN_TOKEN` from `secrets.env` is the bootstrap key and acts as the admin. Public applets skip identity lookup and receive no `x-applet-user`, even when the caller is signed in or uses a key.
...
`ai` in `@std` is chat completions through OpenRouter, and the key never reaches an applet: it is a router secret, `OPENROUTER_API_KEY`, and `@std` calls the router's `AI` capability over RPC, like `Email`. The capability makes the request with `defaultModel` from `packages/router/src/types.ts` when the applet names no model, and returns OpenRouter's body as a `Response`, JSON or a stream of server-sent events, which `@std` parses inside the applet. A refused request throws with OpenRouter's status and message. OpenRouter can also fail after a stream has started, as an `error` event in a 200 body; `ai.stream` throws on it. Because it is RPC and not `fetch()`, an applet with egress `none` can still call it.
